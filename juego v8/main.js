...
function crearBoss(){
	...
	setTimeout(function(){lvl_music.volume = 1}, 2000);
	bosscreado = true;
...
function dibujarJugador(obj) { // Dibujar al jugador (es un método distinto al de dibujar al resto de objetos porque es el único objeto cuyos valores puede alterar el usuario para moverlo)
	...
	} else if (jugador.hp <= jugador.maxhp * 0.4) {
...
function dibujarObjeto(obj){ // Dibujar el resto de objetos
	...
	if(obj.id=="boss"){
		ctx.fillStyle = obj.color;
		ctx.fillRect(obj.x, obj.y, obj.width, obj.height*0.75);
		ctx.beginPath();
		ctx.moveTo(obj.x,obj.y*0.75);
		for(let i=0; i<=10;i++){
			ctx.lineTo(obj.x+(obj.width*(i/10)), obj.y);
			ctx.lineTo(obj.x+(obj.width*(i/10)), obj.y*0.75);
		}
		ctx.closePath();
	}
}
...
let framecounter = 1;
function cambiarFrameFondo(){
	if(framecounter % 2 == 0){
		document.getElementById("pantalla_derrota").style.background = "url('media/images/pantalla_victoria_frame2.jpg')";
		}else{
			document.getElementById("pantalla_derrota").style.background = "url('media/images/pantalla_victoria_frame1.jpg')";
		}
	document.getElementById("pantalla_derrota").style.backgroundSize = "800px 640px"; 
	framecounter++;
	setTimeout(cambiarFrameFondo,1000)
	}
