...
function disparoEnemigo(enemigos){
	...
	enemigos.forEach((enemigo, index) => {
		let disparo = Math.random() <= enemigo.probdisparo ? true : false;

		if (disparo && enemigo.y < (canvas.height/2)) crearProyectilEnemigo(enemigo);

		if (enemigo.id == "boss" && Math.random() <= enemigo.probdisparo*2) disparoColumna(enemigo); 
	});
...
function comprobarVictoria(){
	victoria = enemigos.length == 0 ? true : false;
	if (victoria){
		gameover = true;
		ctx.font = '40px Arial';
		ctx.fillStyle = 'black';
