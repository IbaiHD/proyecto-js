...
function reducirHP(){
	...
	setTimeout(function(){jugador.colorescudo = "black"},100);
...
function crearProyectil(){
	...
	//audioPlay("media/sounds/disparo_jugador.mp3");
...
function crearProyectilEnemigo(enemigo){
	...
	let proyectilenemigo = {
		...
		y: enemigo.y,
		yspeed: jugador.y > enemigo.y ? (jugador.y+(jugador.height/2)-enemigo.y)/enemigo.speedproyectil : -(jugador.y+(jugador.height/2)-enemigo.y)/enemigo.speedproyectil,
...
function crearEnemigo(){
	...
	if (generarenemigo <= masillarate){
		let enemigo = {
			...
			xspeed: 0, yspeed: 4,
			hp: 1,
			probdisparo: shootrate, widthproyectil:6, heightproyectil:6, speedproyectil: 90,
			color: "red",
			moverderecha : true,
		};
		...
		let enemigo = {
			...
			xspeed: 0, yspeed: 2,
			hp: 3,
			probdisparo: shootrate*0.75, widthproyectil:9, heightproyectil:9, speedproyectil: 150,
			color: "pink",
			moverderecha : true,
		};
...
function crearBoss(){
	...
	setTimeout(function(){
		let enemigo = {
			...
			xspeed: 3,
			hp: 10,
			probdisparo: shootrate*2.5, widthproyectil:8, heightproyectil:8, speedproyectil: 80,
			color: "Red",
			moverderecha: true,
		};
...
function dibujarJugador(obj) { // Dibujar al jugador (es un método distinto al de dibujar al resto de objetos porque es el único objeto cuyos valores puede alterar el usuario para moverlo)
	...
	ctx.fillStyle = jugador.colorescudo;

	ctx.lineWidth = 5;

	if (jugador.hp <= jugador.maxhp * 0.20) {
		ctx.strokeStyle = obj.color[2];
	} else if (jugador.hp <= jugador.maxhp * 0.35) {
	...
	if (jugador.hp <= jugador.maxhp * 0.60) {
		ctx.strokeStyle = obj.color[2];
	} else if (jugador.hp < jugador.maxhp) {
		ctx.strokeStyle = obj.color[1];
	} else {
		ctx.strokeStyle = obj.color[0];
	}
...
function ventanaDerrota(){
	...
	setTimeout(stopAudio(lvl_music), 3000);
}
...
function ventanaVictoria(){
		...
		ctx.fillRect(80,40,650,90);
		ctx.fillStyle= 'white';
		ctx.fillText(`¡¡HAS GANADO!!`, 100, 100);
		setTimeout(function () {document.getElementById("menu").style.display = "block";}, 3000);
		setTimeout(stopAudio(lvl_music), 3000);
}
...
function main() {
	...
	if (puntos >= 5) crearBoss();

	// Dibujar puntuación
	ctx.fillStyle = 'white';
	ctx.font = '20px Arial';
	ctx.fillText(`HP: ${jugador.hp}`, 10, 40);
	ctx.fillText(`FondoY: ${gameover}`, 10, 70);
...
audioPlay("media/sounds/level_music.mp3");
crearProyectil();
crearEnemigo();
main();
...
let lvl_music;	
function audioPlay(archivo){
	lvl_music = new Audio(archivo);
	lvl_music.play();
}
...
// 1. La pantalla de inicio bien hecha
// 1.1. Selector de nivel
// 1.2. Tamaño de pantalla, selector de sonido
// 1.3. Selector personaje
// 1.4. Poner fondo al html y que no sea blanco

// 2. Dificultad

// 3. Sonidos (musica de fondo, el resto ya veremos)

// 4. Ponerlo chulo.
// 4.1. Cambiar los cuadraditos por alguna imagen
